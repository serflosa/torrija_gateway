...
msp430_init(void){
    ...
    do{
   	...
   	UCSCTL4 |= SELA__XT1CLK;//Select XT1 source for ACLK
    
    do {                                                                         
        UCSCTL7 &= ~XT2OFFG;                /* XT2 fault flag */  
    }while (UCSCTL7&XT2OFFG);               /* Test XT2 fault flag */   
    
    UCSCTL5 |= DIVS__4; 				//Divide XT2 freq/2 for SMCLK (32 MHz)
...
void __attribute__((interrupt(PORT1_VECTOR))) port1_interrupt(void) {
	int i;
...
void __attribute__((interrupt(PORT2_VECTOR))) port2_interrupt(void) {
	int i;
